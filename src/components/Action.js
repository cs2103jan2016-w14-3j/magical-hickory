    ...
    render() {
        ...
        return (
            <Link to={withPrefix(_.get(action, 'url', null))}
              ...
              className={classNames({'border-black':(border_color==='black'), 'button': (action_style === 'primary') || (action_style === 'secondary'), 'secondary': action_style === 'secondary', 'has-icon': _.get(action, 'has_icon', null)})}>
              {_.get(action, 'has_icon', null) && (
                <Icon {...this.props} icon={action_icon} />
              )}
              <span style={{color:text_color}} className={classNames({'order-first': action_icon_pos === 'right'})}>{_.get(action, 'label', null)}</span>
            </Link>
        );
    }
}